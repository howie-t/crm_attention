"""measure your JND in orientation using a staircse method"""
...
except: # if not there the use a default set
...
dataFile.write('targetSide,orIncrement,correct\n')
...
target = visual.GratingStim(win, sf=1, size=4, mask='gauss',
    ori=expInfo['refOrientation'])
fixation= visual.GratingStim(win, color=-1, colorSpace='rgb', tex=None,
...
# pause until there is a key press
event.waitKeys()
...
for thisIncrement in staircase: # will continue the staircase until it terminates!
    ...
    # add the data to the staircase so it can calcuate the next level
...
feedback1 = visual.TextStim(win, pos=[0,+3], text='mean of final 6 reversals = %.3f' % (approxThreshold))
